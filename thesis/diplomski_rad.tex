...
Slike \ref{fig:dpacorrect} i \ref{fig:dpawrong} pokazuju vrijednost matrice $\Delta$ za jedan oktet $\Psi$ tajnog ključa K. Kao što se može primjetiti, ukoliko je pogođena vrijednost $\Psi$, tada postoji razlika između grupa $G_1$ i $G_2$, a koja rezultira određenim odstupanjima u potrošnji energije. U suprotnom, odstupanja ne postoje, a što daje naslutiti da je pretpostavljena vrijednost $\Psi$ pogrešna.

\todoimg{Primjer traga $T_i$ i vrijednosti u matrici $\Delta$ za ispravno pretpostavljenu vrijednost $\Psi$}

Prethodno opisani postupak pripada u neprofilirane napade analize potrošnje električne energije, odnosno, napadač ne posjeduje kopiju kriptografskog uređaja na kojem bi unaprijed izradio model otkrivanja tajnog ključa. S druge strane analiza potrošnje električne energije može se koristiti u profiliranim napadima klasifikacijom traga, gdje trag može karakterizirati Hammingovu težinu izlaza algoritma ili samu vrijednost. Naime, ukoliko napadač posjeduje kopiju kriptografskog uređaja, on može naučiti model koji prema potrošnji električne energije razlikuje tragove. Tako naučeni model može se zatim iskoristiti na originalnom kriptografskom uređaju. Slika \ref{fig:profiledattack} prikazuje primjer profiliranog napada. Ovaj seminarski rad upravo se bavi profiliranim napadom analize potrošnje električne energije, odnosno problemom klasifikacije tragova. O detaljnijem objašnjenju implementacijskih napada preporuča se pogledati \citep{mangard2008power}. 

\todoimg{Prikaz profiliranog napada.}

\section{DPAcontest skupovi podataka}
\label{sec:dpa_datasets}
\todo{Tko i cilj*}
\todo{Ne zaboravi referencu na stranicu! \citep{dpa}}

Calligraphic letters (e.g., $\mathcal{X}$) denote sets, capital letters (e.g., $X$) denote random variables taking values in these sets, and the corresponding lowercase letters  (e.g., $x$) denote their realizations. 
Let $k^*$ be the fixed secret cryptographic key (byte) and the random variable $T$ the plaintext or ciphertext of the cryptographic algorithm which is uniformly chosen.
The measured leakage is denoted as $X$ and we are particularly interested in multivariate leakage $\vec X = X_1,\ldots,X_D$, where $D$ is the number of time samples or features (attributes) in machine learning terminology.
