...
%--------------------------------------------------------------------------------------%
\chapter{Uvod}
Kriptografski uređaji postali su nezamjenjiv element digitalne infrastrukture modernog društva. Služe za prikrivanje važnih informacija, poput bankovnih transakcija i autorskih djela, i ograničavanje pristupa neovlaštenim osobama. S obzirom na vrijednost informacije koju prikrivaju, česta meta su napada. Znanstvena zajednica provodi etičke napade i analize sigurnosnih sustava, ali i predlaže poboljšanja sigurnosti kriptografskih uređaja, s ciljem da se obznane njihove slabosti i na vrijeme spriječe nedetektirani napadi. Analiza emisija kriptografskog uređaja je jedan primjer napada koji je gotovo nemoguće detektirati.
...
\item
\textbf{DodajPotključ} operatorom ekskluzivno-ili (XOR) matrici stanja dodaje se potključ stvoren iterativnom ekspanzijom originalnog ključa.
\end{enumerate}

Ekspanzijom ključa se između svake dvije iteracije algoritma iz originalnog stvara novi potključ. Pri prvom dodavanju potključa, ekspanzija nije primijenjena i originalni ključ se direktno dodaje stanju. Svi naredni potključevi generiraju se iz potključa prethodne iteracije dodavanjem nelinearne transformacije podbloka prethodne iteracije.
...
\hline
\eqref{eq:dpav2_func1} & 0.587 & 0.582 & 2 \\
\eqref{eq:dpav2_func2} & 0.585 & 0.581 & 3 \\
\eqref{eq:dpav2_func3} & 0.585 & 0.580 & 0 \\
\eqref{eq:dpav2_func4} & 0.585 & 0.579 & 0 \\
\eqref{eq:dpav2_func5} & 0.582 & 0.577 & 3 \\
\end{tabular}
\centering
...
\begin{align}
\label{eq:dpav2_func1}
\begin{split}
&(\cos(x-0.08517142337885375 - 0.8924356742911872 - 3 \\
&\quad -0.08517142337885375 - 0.09079641165917596))^2
\end{split}
\end{align}
\begin{align}
\label{eq:dpav2_func2}
\begin{split}
&elu (elu (\min (1.0,\min (1.0,hsigm (x)) \cdot 0.8102254104210314) \cdot \sin (x)))
\end{split}
\end{align}
\begin{align}
\label{eq:dpav2_func3}
\begin{split}
(\cos(1.0 + x))^2
\end{split}
\end{align}
\begin{align}
\label{eq:dpav2_func4}
\begin{split}
-swish(swish(\cos(x - 0.8949527515835601))) \cdot x
\end{split}
\end{align}
\begin{align}
\label{eq:dpav2_func5}
\begin{split}
&swish (elu (\sin (\cos (x - 1.0 + 0.1607274601962161^9))))
\end{split}
\end{align}
...
Funkcija & Točnost & F1 mjera & Taboo veličina \\
\hline
\eqref{eq:dpav4_func1} & 0.966 & 0.952 & 3 \\
\eqref{eq:dpav4_func2} & 0.966 & 0.950 & 1 \\
\eqref{eq:dpav4_func3} & 0.966 & 0.948 & 2 \\
\eqref{eq:dpav4_func4} & 0.966 & 0.948 & 3 \\
\eqref{eq:dpav4_func5} & 0.962 & 0.947 & 4 \\
\end{tabular}
\centering
\caption{Najboljih 5 pronađenih aktivacijskih funkcija}
...
\begin{align}
\label{eq:dpav4_func1}
\begin{split}
\sin (\min (\sin (\min (x,0.5068783911631836) + 1.0),-0.9990098031722499) + x)
\end{split}
\end{align}
\begin{align}
\label{eq:dpav4_func2}
\begin{split}
\cos (x)
\end{split}
\end{align}
\begin{align}
\label{eq:dpav4_func3}
\begin{split}
ReLU (abs (x) \cdot x)
\end{split}
\end{align}
\begin{align}
\label{eq:dpav4_func4}
\begin{split}
\cos (x) \cdot 0.9815578450294762
\end{split}
\end{align}
\begin{align}
\label{eq:dpav4_func5}
\begin{split}
\frac{x - \exp(x - 1.0)}{1.0}
\end{split}
\end{align}
