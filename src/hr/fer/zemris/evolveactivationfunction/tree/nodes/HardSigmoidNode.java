...
public class HardSigmoidNode extends DerivableNode {
    public static final String NAME = "hsigm";

    public HardSigmoidNode() {
        super(NAME, 1);
    }
    ...
    protected IExecutable<INDArray, INDArray> getExecutable() {
        return (input, node) -> {
            ...
            Nd4j.getExecutioner().execAndReturn(new HardSigmoid(input));
            return input;
        };
    }
    ...
    public IDerivable getDerivable() {
        return (input, node) -> {
            ...
            input = ((DerivableNode) node.getChild(0)).execute(input);
            INDArray out = Nd4j.getExecutioner().execAndReturn(new HardSigmoidDerivative(input));
